...
import { toeflWords } from '../data/toefl';
...
export const fetchBookTitle = async (userId: string, bookId: string): Promise<string> => {
  ...
  if (bookId === 'KAOYAN') return '考研核心词汇';
  if (bookId === 'TOEFL') return '托福核心词汇';
  try {
...
export const fetchWordsForStudy = async (userId: string, book: string): Promise<Word[]> => {
  ...
  if (book === 'CET4' || book === 'CET6') {
    ...
    // Map and deduplicate static words from kaoyan
    kaoyanWords.filter(w => w.book === 'KAOYAN').forEach(w => {
      const docId = `${w.book}_${w.english.replace(/\W/g, '')}`;
      if (!seenIds.has(docId)) {
        seenIds.add(docId);
        allWords.push({
    ...
    // Map and deduplicate static words from toefl
    toeflWords.filter(w => w.book === 'TOEFL').forEach(w => {
      const docId = `${w.book}_${w.english.replace(/\W/g, '')}`;
      if (!seenIds.has(docId)) {
        seenIds.add(docId);
        allWords.push({
