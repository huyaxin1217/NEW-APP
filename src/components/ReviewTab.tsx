...
export function ReviewTab({ outfit, onOpenDressUp, onAddCoins, words, userId, onWordReviewed, currentBook }: ReviewTabProps) {
    ...
    if (currentWord && (currentWord.isPending || currentWord.phonetic === '/pending/') && !isEnrichingCurrentWord) {
      const autoEnrich = async () => {
        ...
        try {
          ...
          // Set isPending to false on failure to prevent infinite loading loop
          const failedWord: Word = {
            ...currentWord,
            phonetic: currentWord.phonetic === '/pending/' ? '/unknown/' : currentWord.phonetic,
            definition: currentWord.definition === '等待 AI 智能生成释义...' ? 'n. 暂无释义 (API 错误)' : currentWord.definition,
            isPending: false
          };
          setQueue(prev => prev.map(w => w.id === currentWord.id ? failedWord : w));
          onWordReviewed(failedWord);
        } finally {
                  ...
                  {choiceOptions.map((opt, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-xl border text-sm transition-all flex flex-col justify-center ${choiceStatus[opt] === 'correct'
              ...
              <div className="flex flex-col items-center justify-center w-full h-full">
                ...
                <form onSubmit={handleSpellingSubmit} className="w-full relative" onClick={e => e.stopPropagation()}>
                  <input
                    ...
                    placeholder="输入英文单词..."
                    className={`w-full text-center text-2xl font-bold p-4 rounded-2xl border-2 outline-none transition-colors ${spellingStatus === 'correct'
