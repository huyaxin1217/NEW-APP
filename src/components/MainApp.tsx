...
export function MainApp({ user }: { user: User }) {
  ...
  useEffect(() => {
    ...
    const initData = async () => {
      try {
        ...
        console.log("fetchedWords length:", fetchedWords.length);
        ...
        console.log("unstudied length:", unstudied.length, "toReview length:", toReview.length);
  ...
  const handleBookChange = async (newBook: string) => {
    ...
    try {
      ...
      console.error("Error changing book:", err);
    } finally {
      setLoading(false);
    }
  };
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'study' && (
              <motion.div key="study" className="h-full" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <StudyTab outfit={outfit} onOpenDressUp={() => setShowDressUp(true)} onAddCoins={handleUpdateCoins} words={newWords} userId={userId} onWordStudied={handleWordStudied} currentBook={currentBook} />
              </motion.div>
            )}
            {activeTab === 'review' && (
              <motion.div key="review" className="h-full" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <ReviewTab outfit={outfit} onOpenDressUp={() => setShowDressUp(true)} onAddCoins={handleUpdateCoins} words={reviewWords} userId={userId} onWordReviewed={handleWordReviewed} currentBook={currentBook} />
              </motion.div>
            )}
            {activeTab === 'library' && (
