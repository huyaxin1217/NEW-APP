...
[
  {
    ...
    "exampleZh": "例句的中文翻译。"
      ...
      // Ultimate public dictionary API fallback if both AI models failed or were missing keys
      if (!parsedList) {
        try {
          console.log("AI lookup failed or keys missing. Falling back to public dictionary API for words...");
          parsedList = await Promise.all(words.map(async (word) => {
            try {
              ...
              if (dictRes.ok) {
                ...
                if (dictData[0]) {
                  ...
                  // Try to find a definition and example
            ...
            return {
              english: word,
              phonetic: `/${word}/`,
              definition: "n. 暂无释义 (请检查本地 API 密钥配置)",
              exampleEn: "No example found.",
              exampleZh: "（请在 AI Studio 设置或本地 .env 配置 API 密钥）"
            };
          }));
        } catch (fallbackErr) {
      ...
      if (parsedList) {
        ...
        throw new Error("Both DeepSeek and Gemini API lookup engines failed, and dictionary fallback failed.");
      }
    } catch (error: any) {
